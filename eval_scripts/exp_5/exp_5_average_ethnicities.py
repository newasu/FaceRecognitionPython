...
average_exp_result = my_util.get_current_path(additional_path=['FaceRecognitionPython_data_store', 'Result', 'average_exp_result'])

# Load label
my_data = pd.read_csv(dataset_path, sep=" ", header=0)
my_label = pd.concat([my_data['gender'], my_data['ethnicity'], (my_data['gender'] + '-' + my_data['ethnicity']), my_data['data_id']], axis=1)
my_label.columns = ['gender', 'ethnicity', 'label', 'id']
my_unique_labels = np.unique(my_label['ethnicity'])
del my_data
...
def do_eval_result(ds, ttmp, ttmp_idx):
    ...
    tmp_1 = my_label.loc[query_idx]['ethnicity']
    query_idx = np.where(my_label['id']==ttmp[ttmp_idx][1])[0][0]
    tmp_2 = my_label.loc[query_idx]['ethnicity']
...
def eval_result(data_id, data_trueY, data_score, data_label_classes):
    ...
    tmp_0 = my_label.loc[tmp_0]['ethnicity'].values
    tmp_1 = tmp.str[1].astype(int) - 1
    tmp_1 = my_label.loc[tmp_1]['ethnicity'].values
...
filename = ['exp_5_alg_euclid', 'exp_5_alg_selmEuclidDistPOS', 'exp_5_alg_selmEuclidMeanPOS', 'exp_5_alg_selmEuclidMultiplyPOS', 'exp_5_alg_selmEuclidSumPOS', 'exp_5_alg_selmRBFDistPOS']
performance_sort_order = ['ascending', 'descending', 'descending', 'descending', 'descending', 'descending']
save_folder = 'exp_5_avg_ethnicities'

for fn_idx in tqdm(range(0, len(filename))):
    save_name = filename[fn_idx] + '_avg_ethnicities'
    data = my_util.exact_run_result_in_directory((exp_result_path + filename[fn_idx] + '/'), exact_list)
    [result_score, eval_score_dict] = exact_result_all_seed(data, my_unique_labels, seed, score_order=performance_sort_order[fn_idx])
