...
for exp_numb in run_exp_round:
    ...
    [_, tmp_kfold_training_idx] = my_util.split_kfold_by_classes(y_gender_training, n_splits=5, random_state=exp_numb)
    [_, tmp_kfold_test_idx] = my_util.split_kfold_by_classes(y_gender_valid, n_splits=5, random_state=exp_numb)
    ...
    [cv_results, avg_cv_results] = welm_model.grid_search_cv_parallel(kfold_training_idx, kfold_test_idx, np.vstack((x_training, x_valid)), np.append(y_gender_training, y_gender_valid), np.append(y_id_training, y_id_valid), param_grid, gridsearch_path, exp_name_seed, cv_run=-1, randomseed=exp_numb, useTF=False, num_cores=num_used_cores)
    #     del tmp_param_grid
