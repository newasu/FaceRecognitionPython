...
# Experiment name
exp_name = ('exp_2_alg_selmRBFDist')
query_exp_name = ('exp_3_alg_selmEuclidDistPOS')
...
for exp_numb in run_exp_kfold:
    ...
    if not my_util.is_path_available(exp_result_path + exp_name_seed + '.npy'):
        ...
        # Grid search
        [cv_results, avg_cv_results] = selm_model.grid_search_cv_parallel(kfold_training_idx, kfold_test_idx, xx, yy, image_id, param_grid, gridsearch_path, query_exp_name_seed, pos_class=pos_class, cv_run=cv_run, randomseed=exp_numb, useTF=training_useTF, combine_rule=combine_rule, num_cores=my_util.limit_cpu_used(cpu_used_perc=0.2))
